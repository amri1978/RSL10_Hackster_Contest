...
static inline ATMO_BOOL_t lfs_pairisnull( const lfs_block_t pair[2] )
{
...
static inline ATMO_BOOL_t lfs_pairsync(
    const lfs_block_t paira[2],
    const lfs_block_t pairb[2] )
{
	return ( paira[0] == pairb[0] && paira[1] == pairb[1] ) ||
...
int lfs_rename( lfs_t *lfs, const char *oldpath, const char *newpath )
{
	...
	ATMO_BOOL_t prevexists = ( err != LFS_ERR_NOENT );
	ATMO_BOOL_t samepair = ( lfs_paircmp( oldcwd.pair, newcwd.pair ) == 0 );
